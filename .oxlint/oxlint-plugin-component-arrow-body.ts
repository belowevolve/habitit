...
const getBodyReplacementRange = (context: Context, body: ESTree.Node): Range => {
...
const isTopLevelComponentArrow = (node: ESTree.ArrowFunctionExpression): boolean => {
  const { parent } = node;
...
const isTopLevelFunctionDeclaration = (node: ESTree.FunctionDeclaration): boolean => {
...
const getSingleReturnStatement = (block: ESTree.BlockStatement): ESTree.ReturnStatement | null => {
...
const toBlockBodyText = (context: Context, expressionNode: ESTree.Node): string => {
  const exprText = context.sourceCode.getText(unwrapParenthesized(expressionNode));
  return `{ return ${exprText}; }`;
};

const toConciseBodyText = (context: Context, expressionNode: ESTree.Node): string => {
...
const componentAwareArrowBodyStyleRule = defineRule({
  createOnce(context: Context) {
    const options = Array.isArray(context.options)
      ...
      : {};
    const nonComponentStyle: RuleMode = options.nonComponentStyle ?? "as-needed";
    const requireReturnForObjectLiteral = options.requireReturnForObjectLiteral ?? false;
    ...
    return {
      ArrowFunctionExpression(node: ESTree.ArrowFunctionExpression) {
        ...
        if (nonComponentStyle === "always") {
          ...
          context.report({
            ...
            message: "Unexpected expression body; wrap arrow function body in braces.",
            node: body,
          });
          return;
        }
        ...
        context.report({
          fix(fixer: Fixer) {
            return fixer.replaceText(body, toConciseBodyText(context, returnArgument));
          },
          message:
      ...
      FunctionDeclaration(node: ESTree.FunctionDeclaration) {
        const { id } = node;
        if (!id || !isTopLevelFunctionDeclaration(node)) {
          return;
        }

        const { name } = id;
        const idRange = context.sourceCode.getRange(id);
        const bodyRange = context.sourceCode.getRange(node.body);
        const { text } = context.sourceCode;

        const signature = text.slice(idRange[1], bodyRange[0]).trimEnd();
        const bodyText = text.slice(bodyRange[0], bodyRange[1]);
        const arrowFn = `const ${name} = ${signature} => ${bodyText};`;
        ...
        context.report({
          fix(fixer: Fixer) {
            if (isExportNamed && parent) {
              return fixer.replaceTextRange(
                context.sourceCode.getRange(parent),
                `export ${arrowFn}`,
              );
            }
            return fixer.replaceTextRange(context.sourceCode.getRange(node), arrowFn);
          },
          message:
            "Top-level component function declarations must use arrow expressions: const Foo = () => { ... };",
          node,
        });
      },
    };
  ...
  meta: {
    docs: {
      description: "Apply arrow-body-style with top-level component override in TSX.",
    },
    fixable: "code",
    schema: [
      {
